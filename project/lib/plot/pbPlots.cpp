...
void DrawFilledpb_RectangleWithBorder(RGBABitmapImage *image, double x, double y, double w, double h, RGBA *borderColor, RGBA *fillColor){
  if(h > 0.0 && w > 0.0){
    DrawFilledpb_Rectangle(image, x, y, w, h, fillColor);
    Drawpb_Rectangle1px(image, x, y, w, h, borderColor);
  }
}
...
bool pb_RectanglesOverlap(pb_Rectangle *r1, pb_Rectangle *r2){
  bool overlap;
...
pb_Rectangle *Createpb_Rectangle(double x1, double y1, double x2, double y2){
  pb_Rectangle *r;
  r = Allocate<pb_Rectangle>();
  r->x1 = x1;
  r->y1 = y1;
  r->x2 = x2;
  r->y2 = y2;
...
void Copypb_RectangleValues(pb_Rectangle *rd, pb_Rectangle *rs){
  rd->x1 = rs->x1;
  rd->y1 = rs->y1;
  rd->x2 = rs->x2;
  rd->y2 = rs->y2;
}
void DrawXLabelsForPriority(double p, double xMin, double oy, double xMax, double xPixelMin, double xPixelMax, NumberReference *nextpb_Rectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas, vector<double> *xGridPositions, StringArrayReference *xLabels, NumberArrayReference *xLabelPriorities, vector<pb_Rectangle*> *occupied, bool textOnBottom){
  ...
  for(i = 0.0; i < xLabels->stringArray->size(); i = i + 1.0){
    if(xLabelPriorities->numberArray->at(i) == p){
      ...
      for(j = 0.0; j < nextpb_Rectangle->numberValue; j = j + 1.0){
        currentOverlaps = currentOverlaps || pb_RectanglesOverlap(r, occupied->at(j));
      }

      if( !currentOverlaps  && p == 1.0){
        DrawText(canvas, r->x1 + padding, r->y1 + padding, text, gridLabelColor);

        Copypb_RectangleValues(occupied->at(nextpb_Rectangle->numberValue), r);
        nextpb_Rectangle->numberValue = nextpb_Rectangle->numberValue + 1.0;
      }
...
void DrawYLabelsForPriority(double p, double yMin, double ox, double yMax, double yPixelMin, double yPixelMax, NumberReference *nextpb_Rectangle, RGBA *gridLabelColor, RGBABitmapImage *canvas, vector<double> *yGridPositions, StringArrayReference *yLabels, NumberArrayReference *yLabelPriorities, vector<pb_Rectangle*> *occupied, bool textOnLeft){
  bool overlap, currentOverlaps;
...
double MapXCoordinateBasedOnSettings(double x, ScatterPlotSettings *settings){
  ...
  pb_Rectangle *boundaries;

  boundaries = Allocate<pb_Rectangle>();
...
bool DrawScatterPlotFromSettings(RGBABitmapImageReference *canvasReference, ScatterPlotSettings *settings, StringReference *errorMessage){
  ...
  NumberReference *nextpb_Rectangle, *x1Ref, *y1Ref, *x2Ref, *y2Ref, *patternOffset;
  ...
  if(success){
    ...
    Drawpb_Rectangle1px(canvas, xPixelMin, yPixelMin, xLengthPixels, yLengthPixels, settings->gridColor);
...
if(settings->yAxisLeft){
    ...
    /* Labels */
    occupied = Allocate<pb_Rectangle*>(xLabels->stringArray->size() + yLabels->stringArray->size());
    for(i = 0.0; i < occupied->size(); i = i + 1.0){
      occupied->at(i) = Createpb_Rectangle(0.0, 0.0, 0.0, 0.0);
    }
    nextpb_Rectangle = CreateNumberReference(0.0);
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      ...
      DrawXLabelsForPriority(i, xMin, originYPixels, xMax, xPixelMin, xPixelMax, nextpb_Rectangle, gridLabelColor, canvas, xGridPositions, xLabels, xLabelPriorities, occupied, textOnBottom);
    }
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      ...
      DrawYLabelsForPriority(i, yMin, originXPixels, yMax, yPixelMin, yPixelMax, nextpb_Rectangle, gridLabelColor, canvas, yGridPositions, yLabels, yLabelPriorities, occupied, textOnLeft);
    }
...
void ComputeBoundariesBasedOnSettings(ScatterPlotSettings *settings, pb_Rectangle *boundaries){
  ScatterPlotSeries *sp;
...
bool DrawBarPlotFromSettings(RGBABitmapImageReference *canvasReference, BarPlotSettings *settings, StringReference *errorMessage){
  ...
  vector<pb_Rectangle*> *occupied;
  NumberReference *nextpb_Rectangle;
  ...
  if(success){
    ...
    /* Labels */
    occupied = Allocate<pb_Rectangle*>(yLabels->stringArray->size());
    for(i = 0.0; i < occupied->size(); i = i + 1.0){
      occupied->at(i) = Createpb_Rectangle(0.0, 0.0, 0.0, 0.0);
    }
    nextpb_Rectangle = CreateNumberReference(0.0);
    ...
    for(i = 1.0; i <= 5.0; i = i + 1.0){
      DrawYLabelsForPriority(i, yMin, xPixelMin, yMax, yPixelMin, yPixelMax, nextpb_Rectangle, gridLabelColor, canvas, yGridPositions, yLabels, yLabelPriorities, occupied, true);
    }
    ...
    for(n = 0.0; n < bs; n = n + 1.0){
      for(s = 0.0; s < ss; s = s + 1.0){
        ...
        if(settings->barBorder){
          DrawFilledpb_RectangleWithBorder(canvas, Round(x), Round(y), Round(w), Round(h), GetBlack(), barColor);
        }else{
          DrawFilledpb_Rectangle(canvas, Round(x), Round(y), Round(w), Round(h), barColor);
        }
...
void Drawpb_Rectangle1px(RGBABitmapImage *image, double x, double y, double width, double height, RGBA *color){
...
void DrawFilledpb_Rectangle(RGBABitmapImage *image, double x, double y, double w, double h, RGBA *color){
  double i, j;
...
void DrawLineBresenhamsAlgorithmThick(RGBABitmapImage *canvas, double x1, double y1, double x2, double y2, double thickness, RGBA *color){
  ...
  if(thickness >= 3.0){
    ...
    DrawFilledpb_Rectangle(canvas, x, y, 2.0, 2.0, color);
